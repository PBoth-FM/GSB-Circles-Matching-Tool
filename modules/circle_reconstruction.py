...
def reconstruct_circles_from_results(results, original_circles=None, use_standardized_metadata=False):
    ...
    for circle_id in unique_circles:
        ...
        always_hosts = 0
        sometimes_hosts = 0
        if 'host' in members_df.columns:
            for _, row in members_df.iterrows():
                host_status = str(row.get('host', '')).lower()
                if 'always' in host_status:
                    always_hosts += 1
                elif 'sometimes' in host_status:
                    sometimes_hosts += 1

        # Count new members and continuing members
        new_members = 0
        ...
        if always_hosts > 0:
            print(f"  Host breakdown: {always_hosts} always, {sometimes_hosts} sometimes")
        ...
        if circle_id.startswith('IP-NEW') or circle_id.startswith('V-'): # Also virtual circles
            # We should set member_count to the actual number of unique members
            if member_count < len(member_ids):
                member_count = len(member_ids)
                print(f"  🔧 NEW/VIRTUAL CIRCLE FIX: Set member_count for {circle_id} to {member_count} based on members list")
        ...
        is_virtual = isinstance(circle_id, str) and ('VO-' in circle_id or 'V-' in circle_id) # Updated to check for 'V-' as well
                    ...
                    if 'Phoenix/Scottsdale/Arizona' in proposed_value and (
                        ...
                        if not correct_subregion:
                            ...
                            if circle_id and '-' in circle_id:
                                ...
                                correct_subregion = "Unknown"
                                print(f"  ⚠️ No valid circle ID format, using default subregion 'Unknown'")
        ...
        for prop in ['region', 'subregion']:
            # Skip if already set for new circles and it's not 'Unknown'
        ...
        if special_region_code:
            ...
            # Define available subregions and meeting times for deterministic assignment for new circles
            # These are used as fallbacks when specific data is missing
            available_subregions = ['Central', 'East', 'North', 'South', 'West', 'Valley']
            available_meeting_times = ['Monday (Evenings)', 'Tuesday (Evenings)', 'Wednesday (Evenings)',
                                      'Thursday (Evenings)', 'Friday (Evenings)', 'Saturday (Mornings)']
            ...
            try:
                ...
                if is_continuing_circle:
                    ...
                    # For continuing circles, check if the current subregion needs fixing (missing, unknown, or has Phoenix)
                    current_subregion = extracted_props.get('subregion', 'Unknown')
                    needs_subregion_fix = (
                        current_subregion == 'Unknown' or
                        not current_subregion or
                        ('Phoenix' in current_subregion and circle_id.startswith('IP-PSA-')) # Specific fix for PSA
                    )
                    ...
                    if needs_subregion_fix:
                        ...
                        if continuing_subregions:
                            ...
                            extracted_props['subregion'] = available_subregions[0] # Fallback to first in list
    ...
    for i, row in circles_df.iterrows():
        ...
        if circle_id in member_count_from_list:
            ...
            if circle_id.startswith('IP-NEW') or (is_new and not is_continuing): # Check if it's a new circle
                ...
                # Even if the counts match, always set member_count to new_members for new circles if new_members > 0
                elif new_members_count > 0 and member_count != new_members_count:
                    old_count = member_count
                ...
                # Handle cases where member_count is 1 but actual members list is larger
                elif member_count == 1 and actual_count > 1:
                    old_count = member_count
                    circles_df.at[i, 'member_count'] = actual_count
                    new_count_fixed += 1
                    print(f"  🔧 DIRECT OVERRIDE FOR NEW CIRCLE: {circle_id} member_count forced from {old_count} to {actual_count} (based on members list)")
    ...
    for i, row in circles_df.iterrows():
        ...
        if circle_id.startswith('IP-NEW') or (row.get('is_new_circle', False) and not row.get('is_existing', False)):
            member_count = row.get('member_count', 0)
            new_members_count = row.get('new_members', 0)
            actual_members_list_count = len(row.get('members', []))

            # For new circles, member_count should be at least new_members_count and actual_members_list_count
            # It should represent the total members correctly.
            correct_count = max(new_members_count, actual_members_list_count)

            if member_count != correct_count:
                old_count = member_count
                circles_df.at[i, 'member_count'] = correct_count
                final_fixed += 1
                print(f"  🚨 FINAL FIX: NEW circle {circle_id} member_count forced from {old_count} to {correct_count}")
                print(f"    Reason: new_members={new_members_count}, members_list_count={actual_members_list_count}")
